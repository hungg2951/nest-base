...
function formatDefault(
  ...
  // Object/array → JSON literal, cast đúng kiểu json/jsonb
  if (typeof value === 'object' && value !== null) {
    const json = JSON.stringify(value).replace(/'/g, "''");
    const cast = columnType === 'jsonb' ? '::jsonb' : '::json';
    return `'${json}'${cast}`;
  }
...
function buildColumnSQL(
  ...
  if (definition.required && !definition.primary) {
    parts.push('NOT NULL');
  }
  ...
  if (definition.default !== undefined) {
    parts.push(`DEFAULT ${formatDefault(definition.default, definition.type)}`);
